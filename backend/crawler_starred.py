...
class GraphQLGitHub:
    ...
    def __get_viewer_starred(self) -> List[Tuple[str, str]]:
        ...
        has_next_page = True
        while has_next_page:
            query = 'query { viewer { starredRepositories(first: 100, after: "%s") { ' \
                    'nodes { nameWithOwner } edges { starredAt } pageInfo { endCursor hasNextPage } } } }' % cursor
            starred_repositories = self.__query(query)['data']['viewer']['starredRepositories']
            for node, edge in zip(starred_repositories['nodes'], starred_repositories['edges']):
