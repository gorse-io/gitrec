      ...
      <div v-else>
        ...
        <ul class="pagination">
          <li :class="{'disabled': currentPage === 1, 'waves-effect': currentPage > 1}">
            <a @click="currentPage--"><i class="material-icons">chevron_left</i></a>
          </li>
          <li v-for="index in numPage" :key="index"
            :class="index === currentPage ? ['active','blue','darken-1'] : ['waves-effect']">
            <a @click="currentPage = index">{{ index }}</a>
          </li>
          <li :class="{'disabled': currentPage === numPage, 'waves-effect': currentPage < numPage}">
            <a @click="currentPage++"><i class="material-icons">chevron_right</i></a>
          </li>
        </ul>
      </div>
    </div>
